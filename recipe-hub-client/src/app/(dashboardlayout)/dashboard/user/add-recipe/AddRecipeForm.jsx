...
export default function AddRecipeForm({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // console.log("Submitting Data:", finalRecipeData);
    const res = await addRecipe(finalRecipeData);
    // console.log("call after api", res);
    if (res.status === true) {
      toast.success(`${res.message}`);
    }

    if (user?.limit < 2) {
      const res = await updateuserAddrecipeLimit(user?.id);
      console.log(res);
    }
    // You can make your backend API call here
    setLoading(false);
  };
